use crate::sys::ImFontGlyphRangesBuilder;
...
use imgui::{FontConfig, FontSource, FontGlyphRanges};

const CN_font_path:&str  = "C:/Windows/Fonts/simhei.ttf";
...
fn setup_fonts(ctx: &mut imgui::Context) {
    // Adjust this path to your actual .ttf file location
    let font_data = std::fs::read(CN_font_path).expect("Failed to read font");
    ...
    ctx.fonts().add_font(&[
        FontSource::TtfData {
            ...
            config: Some(FontConfig {
                // Use merge_mode if you're adding multiple fonts
                rasterizer_multiply: 1.75,
                oversample_h: 2,
                oversample_v: 2,
                //glyph_ranges: FontGlyphRanges::japanese(), // Includes Chinese + Kana
                glyph_ranges: FontGlyphRanges::from_slice(&[
                    0x0020, 0x00FF, // Basic Latin
                    0x3000, 0x30FF, // Punctuation + Katakana
                    0x4E00, 0x9FFF, // Common CJK Unified Ideographs (Hanzi/Kanji)
                    0,
                ]),
                ..FontConfig::default()
            }),
